...
export function SongResult({ song, onTryAnother }: SongResultProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto mt-6 md:mt-8 px-4">
      {/* Confetti effect (brief celebration) */}
      {showConfetti && (
        <div className="fixed inset-0 pointer-events-none z-50">
          <div className="absolute top-1/4 left-1/4 w-3 h-3 md:w-4 md:h-4 bg-purple-500 rounded-full animate-ping"></div>
          <div className="absolute top-1/3 right-1/4 w-2.5 h-2.5 md:w-3 md:h-3 bg-pink-500 rounded-full animate-ping" style={{animationDelay: '0.2s'}}></div>
          <div className="absolute bottom-1/3 left-1/3 w-2.5 h-2.5 md:w-3 md:h-3 bg-blue-500 rounded-full animate-ping" style={{animationDelay: '0.4s'}}></div>
        </div>
      )}

      {/* Success badge */}
      <div className="text-center mb-5 md:mb-6">
        <div className="inline-flex items-center gap-2 px-5 md:px-6 py-2.5 md:py-3 rounded-full glass-card">
          <svg className="w-5 h-5 md:w-6 md:h-6" style={{color: 'var(--success)'}} fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          <span className="font-semibold text-white text-sm md:text-base">Song Found!</span>
        </div>
      </div>
      ...
      <div className="glass-card overflow-hidden hover-lift">
        ...
        <div className="p-5 md:p-6 lg:p-8">
          ...
          <p className="text-lg md:text-xl lg:text-2xl mb-3 md:mb-4" style={{color: 'var(--text-secondary)'}}>
            {song.artist}
          </p>

          {song.album && (
            <p className="text-sm md:text-base mb-1.5 md:mb-2" style={{color: 'var(--text-secondary)'}}>
              <span className="opacity-60">Album:</span> {song.album}
            </p>
          )}
          {song.releaseDate && (
            <p className="text-xs md:text-sm mb-5 md:mb-6" style={{color: 'var(--text-disabled)'}}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 md:gap-3">
                {song.externalLinks.spotify && (
                  <a
                    ...
                    className="flex items-center justify-center gap-2 md:gap-3 px-4 md:px-6 py-3 md:py-4 rounded-xl font-semibold text-sm md:text-base transition-all hover-lift"
                    style={{
                      background: '#1DB954',
                      color: 'white',
          ...
          <button
            ...
            className="mt-8 md:mt-10 w-full py-3.5 md:py-4 px-5 md:px-6 rounded-xl font-semibold text-sm md:text-base transition-all hover-lift"
            style={{
